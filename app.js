function inicia_jogo() {
    ...
    //muda a classe das peças pretas, para mostrar imgens das pecas
    ...
    //muda a classe das peças brancas, para mostrar imgens das peças
    ...
    //Array de posições do tabuleiro
    cria_array();
    function cria_array() {
        var x, y;
        ...
        for (x = 1; x <= 8; x++) {
            ...
            for (y = 1; y <= 8; y++) {
                ...
                peca[x][y] = new Array();       //Peças que não começam ficam nulas
                peca[x][y]['peca'] = false; 		
                peca[x][y]['cor'] = false;			
    ...
    ///Array para movimentar as peças
    movimenta = new Array();
...
function possiveis_movimentos() {
    var x, y;
    var c = 0; //contador para arrays possíveis
...
function seleciona(x, y) {
    ...
    if ((movimenta['selecionada']['x'] == 0 || peca[x][y]['cor'] == movimenta['selecionada']['cor']) && peca[x][y]['cor'] == vez) {
        if (movimenta['selecionada']['x'] != 0) {
            volta_fundo(); //volta a cor de fundo padrão
        }
        ...
        //Troca de peça quando o peão chegar do outro lado
        ...
        if (peca[x][y]['cor'] != movimenta['selecionada']['cor']) {
            ...
            movimenta['selecionada']['x'] = 0;	//selecionada x recebe 0 (para na próxima ver que é o primeiro movimento)
            movimenta['selecionada']['y'] = 0;	//selecionada y recebe 0 (pra na próxima ver que é o primeiro movimento)
            movimenta['selecionada']['peca'] = "0";	//selecionada peça recebe 0 (para na próxima ver que é o primeiro movimento)
